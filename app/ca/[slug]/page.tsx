...
export async function generateMetadata({
  ...
  if (!page) {
    return {
      title: "Page Not Found | PixPassVisa Canada",
    };
  }

  const canonicalUrl = `https://www.pixpassvisa.com/ca/${page.slug}`;
...
export default async function CASlugPage({ params }: PageProps) {
  ...
  const softwareSchema = {
    ...
    name: page.title,
    url: `https://www.pixpassvisa.com/ca/${page.slug}`,
    description: page.description,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: "https://www.pixpassvisa.com/ca",
      },
      {
        "@type": "ListItem",
        position: 3,
        name: page.h1 || page.title,
        item: `https://www.pixpassvisa.com/ca/${page.slug}`,
      },
    ],
  };
              ...
              <div className="sticky top-24 bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 text-[11px] space-y-2 font-medium text-slate-500">
                  <div className="flex items-center gap-1.5 text-red-700 font-bold">
                    <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> IRCC &amp; Passport Program Compliant
                  </div>
                  <div className="flex items-center gap-1.5 text-red-700 font-bold">
                    <Zap className="w-3.5 h-3.5 shrink-0" /> 30-Second Result
                  </div>
                  <div className="flex items-center gap-1.5 text-red-700 font-bold">
                    <Lock className="w-3.5 h-3.5 shrink-0" /> Deleted in 24 Hours
                  </div>
                </div>
              </div>
            </aside>
